		...
		<div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-[#0b0b14] to-black p-6">
			<Card className="max-w-2xl w-full border-white/10 bg-gradient-to-b from-zinc-900/40 to-black/60">
				<CardHeader className="text-center space-y-3">
					...
					<CardTitle className="text-2xl text-white">Start your Starter plan</CardTitle>
					<CardDescription className="text-gray-300">
						Enjoy a 30-day free trial. No charge today. £15/month after trial. Cancel anytime.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-6">
					<div className="overflow-hidden rounded-xl border border-white/10">
						<img src="/favicon.svg" alt="Starter" className="w-full h-40 object-contain bg-gradient-to-r from-purple-600/10 to-blue-600/10" />
					</div>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<Button onClick={startTrial} disabled={loading !== null} className="h-12">
							{loading === 'trial' ? 'Redirecting…' : 'Start 30-day Free Trial'}
						</Button>
						<Button onClick={startPaid} variant="secondary" disabled={loading !== null} className="h-12 bg-white/10 hover:bg-white/20">
							{loading === 'paid' ? 'Redirecting…' : 'Pay £15/month now'}
						</Button>
					</div>
