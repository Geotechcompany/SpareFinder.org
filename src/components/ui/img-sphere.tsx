      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/20">
          <img
            ...
            onError={(e) => {
              // Avoid providers that commonly fail CORS in browsers (source.unsplash, pravatar)
              // 1) Deterministic UI Avatars based on image id or alt text
              const nameSeed =
                (image.alt && image.alt.substring(0, 24)) || image.id || 'User';
              const fallback1 = `https://ui-avatars.com/api/?background=3B82F6&color=ffffff&size=400&name=${encodeURIComponent(
                nameSeed
              )}`;
              // 2) Local placeholder (always succeeds)
              const fallback2 = `/placeholder.svg`;
              const current = e.currentTarget.src;
              if (current !== fallback1) {
                e.currentTarget.src = fallback1;
              } else if (current !== fallback2) {
        ...
        >
          <div className="relative aspect-square">
            <img
              ...
              onError={(e) => {
                const nameSeed =
                  (selectedImage.alt && selectedImage.alt.substring(0, 24)) ||
                  selectedImage.id ||
                  'User';
                const fallback1 = `https://ui-avatars.com/api/?background=3B82F6&color=ffffff&size=800&name=${encodeURIComponent(
                  nameSeed
                )}`;
                const fallback2 = `/placeholder.svg`;
                const current = e.currentTarget.src;
