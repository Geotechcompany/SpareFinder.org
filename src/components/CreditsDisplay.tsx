    ...
    try {
      ...
      if (response.success) {
        ...
        if (unlimited) {
          ...
          onCreditChange?.(Number.MAX_SAFE_INTEGER);
        } else if (typeof (response as any).credits === "number") {
  ...
  return (
    <motion.div
      ...
      className={`inline-flex items-center bg-gradient-to-r ${getStatusColor()} 
                  backdrop-blur-sm rounded-full border ${sizes.container} ${
        sizes.gap
      } ${className}`}
    >
      {getIcon()}
      ...
      <span className={`text-white ${sizes.credit}`}>
        {loading ? "..." : credits === "unlimited" ? "∞" : credits}
      </span>
      ...
      {!loading && (credits === null || credits === 0) && !planActive && (
        <Button
          variant="outline"
          size="sm"
          className={`ml-2 bg-white/5 hover:bg-white/10 text-white border-white/20 ${
