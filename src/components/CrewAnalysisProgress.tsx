 ...
 * 
 * Shows real-time progress of AI Deep Research in history cards
...
import { CREW_STAGES, getStageDisplayName, getStageIcon } from '@/services/aiAnalysisCrew';
...
export const CrewAnalysisProgress = ({
  ...
  const isAnalyzing = status === 'analyzing' || status === 'in_progress' || status === 'pending' || status === 'processing';
  const isCompleted = status === 'completed';
  const isError = status === 'error' || status === 'failed';

  const stages = [
    { key: CREW_STAGES.PART_IDENTIFIER, name: 'Part Identification', icon: '🔬', inProgress: 'Part Being Identified...', completed: '✅ Part Identified' },
    { key: CREW_STAGES.RESEARCH, name: 'Technical Research', icon: '📊', inProgress: 'Technical Research Ongoing...', completed: '✅ Technical Research Completed' },
    { key: CREW_STAGES.SUPPLIER_FINDER, name: 'Supplier Discovery', icon: '🏪', inProgress: 'Supplier Search In Progress...', completed: '✅ Suppliers Identified' },
    { key: CREW_STAGES.REPORT_GENERATOR, name: 'Report Compilation', icon: '📄', inProgress: 'Compiling Structured Report...', completed: '✅ Comprehensive Report Generated' },
    { key: CREW_STAGES.EMAIL_AGENT, name: 'Completion & Delivery', icon: '📧', inProgress: 'Finalizing Output for Delivery...', completed: '✅ Analysis Ready & Delivery Confirmed' },
  ];
        ...
        {isAnalyzing && (
          <>
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-400">AI Deep Research</span>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-xs text-gray-400">
                ...
                <span>{currentStage ? getStageDisplayName(currentStage) : 'Initializing...'}</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
            ...
            {isExpanded && (
              <div className="space-y-1.5 pt-2 border-t border-gray-700/50">
                {stages.map((stage, index) => {
                  ...
                  const displayText = stageStatus === 'in_progress' 
                    ? stage.inProgress 
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        ${stageStatus === 'in_progress' ? 'bg-blue-600/20 text-blue-300' : ''}
                        ${stageStatus === 'completed' ? 'text-green-400' : ''}
                        ${stageStatus === 'pending' ? 'text-gray-500 opacity-60' : ''}
                      `}
                    >
                      ...
                      {stageStatus !== 'completed' && <span className="text-xs">{stage.icon}</span>}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge
          variant={isAnalyzing ? 'default' : isCompleted ? 'success' : 'destructive'}
          className={
            isAnalyzing
              ? 'bg-blue-500'
              : isCompleted
          ...
          return (
            <div
              ...
              className={`
                ...
                ${stageStatus === 'in_progress' ? 'bg-blue-50 border-blue-200 dark:bg-blue-950/30 dark:border-blue-800' : ''}
                ${stageStatus === 'completed' ? 'bg-green-50 border-green-200 dark:bg-green-950/30 dark:border-green-800' : ''}
                ${stageStatus === 'pending' ? 'bg-gray-50 border-gray-200 dark:bg-gray-900/30 dark:border-gray-700 opacity-60' : ''}
              `}
            >
              <div className="flex-shrink-0">
                {stageStatus === 'in_progress' && (
              ...
              <div className="flex items-center gap-2 flex-1">
                {stageStatus !== 'completed' && <span className="text-sm">{stage.icon}</span>}
