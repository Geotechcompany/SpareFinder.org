"""
Updated main.py with database storage for jobs
...
async def list_jobs():
    try:
        ...
        jobs = db_job_store.list_jobs(limit=1000)  # Adjust limit as needed
...
async def get_job_statistics():
    """Get job statistics"""
    try:
        stats = db_job_store.get_job_statistics()
...
async def get_job(job_id: str):
    ...
    try:
        job = db_job_store.load_job(job_id)
        if job:
...
async def delete_job(job_id: str):
    """Delete a job by ID"""
    try:
        if db_job_store.delete_job(job_id):
...
# Replace calls to save_job_snapshot() with db_job_store.save_job()

# Example: In your analyze_part endpoint, replace:
# save_job_snapshot(job_id, result)
# with:
# db_job_store.save_job(job_id, result)
