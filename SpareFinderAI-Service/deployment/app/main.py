...
async def search_by_keywords(payload: KeywordSearchRequest):
    try:
        ...
        system_prompt = (
            "You are an expert Engineering spares parts search assistant. Given a list of keywords, "
...
async def _run_keyword_search_job(job_id: str, keyword_list: List[str], user_email: Optional[str]) -> None:
    try:
        ...
        system_prompt = (
            "You are SpareFinder AI. Given Engineering spares keywords, return a concise JSON with 'results'[] and optional 'markdown' string."
        )
        user_prompt = (
            f"Find Engineering spares parts related to: {', '.join(normalized)}. Return fields: name, category, manufacturer, price, availability, part_number."
        )
