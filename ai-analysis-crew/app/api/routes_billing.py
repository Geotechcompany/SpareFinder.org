...
@router.get("/cron/trial-ended")
async def cron_trial_ended():
    """
    Cron: find users whose free trial has ended (status=trialing, current_period_end < now),
    exclude admin/super_admin, set subscription to canceled, and send renewal email.
    Call daily (e.g. cron-job.org). Does not touch admin accounts.
    """
    now = datetime.utcnow()
    ...
    try:
        ...
        return api_error(f"Failed to fetch trial-ended subscriptions: {e}", status_code=500)
    if not subs:
        return api_ok(
            data={
                "deactivated": 0,
                ...
                "errors": 0,
                "skipped_admin": 0,
                "message": "No trial-ended subscriptions found.",
            }
        )
    ...
    for sub in subs:
        ...
        if uid_str in admin_user_ids:
            ...
            continue
        sub_id = sub.get("id")
        tier = sub.get("tier") or sub.get("plan_type") or "free"
        try:
        ...
        if not email:
            ...
            continue
        try:
            if send_trial_ended_renew_email(to_email=email, plan_name=tier or "free trial"):
                emails_sent += 1
            else:
                errors += 1
        except Exception as e:
